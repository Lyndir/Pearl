// See http://github.com/lhunath/uicolor-utilities/
...
- (UIColor *)colorByMultiplyingByRed:(CGFloat)red green:(CGFloat)green blue:(CGFloat)blue alpha:(CGFloat)alpha {
    ...
    return [UIColor colorWithRed:fmaxf(0.0f, fminf(1.0f, r * red))
                           green:fmaxf(0.0f, fminf(1.0f, g * green)) 
                            blue:fmaxf(0.0f, fminf(1.0f, b * blue))
                           alpha:fmaxf(0.0f, fminf(1.0f, a * alpha))];
}
...
+ (UIColor *)colorWithComponentString:(NSString *)stringToConvert {
...
+ (UIColor *)colorWithString:(NSString *)stringToConvert {
    ...
    UIColor *color      = [self colorWithHexString:stringToConvert];
    if (!color) color   = [self colorWithName:stringToConvert];
    if (!color) color   = [self crayonWithName:stringToConvert];
    if (!color) color   = [self colorWithComponentString:stringToConvert];
...
+ (UIColor *)colorWithHexString:(NSString *)stringToConvert {
    ...
    if ([stringToConvert length] == 3) {
        // Convert short-notation (eg. C32) to long (CC3322).
        NSMutableString *newString = [NSMutableString stringWithCapacity:6];
        for (NSUInteger i = 0; i < [stringToConvert length]; ++i) {
            [newString appendString:[stringToConvert substringWithRange:NSMakeRange(i, 1)]];
            [newString appendString:[stringToConvert substringWithRange:NSMakeRange(i, 1)]];
        }
